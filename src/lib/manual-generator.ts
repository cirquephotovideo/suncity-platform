...
const AUDIENCES: Record<Audience, { sections: Section[]; tone: string; intro: string }> = {
  user: {
    ...
    intro: 'Bienvenue dans Sun City Paris ! Ce manuel te guide à travers toutes les fonctionnalités du site pour que tu te sentes chez toi.'
  },
  admin: {
...
export async function generateManual(audience: Audience): Promise<GeneratedManual> {
  ...
  for (const sec of cfg.sections) {
    ...
    if (!quotaExhausted) {
      const prompt = `Tu rédiges UNE section d'un manuel utilisateur pour le site Sun City Paris (réseau social inclusif religieux LGBT+, https://gld.pixeeplay.com).
...
<div class="container">
  ...
  <div class="meta">
    ...
    Régénéré 2×/jour pour rester à jour. — Sun City Paris · gld.pixeeplay.com
  </div>
</div>
</body>
</html>`;
...
function renderMarkdown(audience: Audience, version: string, intro: string, sections: any[]): string {
  ...
  let md = `# 🌈 Sun City Paris — Manuel ${labels[audience]}\n\nv${version}\n\n${intro}\n\n## Sommaire\n\n`;
...
function renderVideoScript(audience: Audience, sections: any[]): string {
  ...
  script += `---\n\n## 🎬 OUVERTURE (0:00 - 0:15)\n\n`;
  script += `**Voix off :**\n> Bienvenue dans Sun City Paris. Voici un tour complet du site.\n\n`;
