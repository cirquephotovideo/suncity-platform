  ...
  for (let w = 0; w < 52; w++) {
    ...
    try {
      const r = await generateText(
        `Génère un titre court et accrocheur (max 60 chars) + un preheader (max 90 chars) pour une newsletter du site Sun City Paris (réseau social inclusif religieux LGBT+) sur le thème : "${theme}". Date : semaine ${weekNum} de ${yr}. Réponds UNIQUEMENT en JSON strict : {"title":"...","preheader":"..."}`
      );
      await bumpQuota(1);
