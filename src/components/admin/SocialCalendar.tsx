...
function PostEditor({ post, onClose, onSaved }: {
  ...
  async function aiSuggest() {
    ...
    const r = await fetch('/api/ai/text', {
      ...
      body: JSON.stringify({
        prompt: `Rédige un post court (≤280 caractères) pour ${channels.join(', ')} pour le mouvement Sun City Paris. Inclusif, lumineux, hashtag #GodLovesDiversity.`,
        system: 'Tu es community manager de Sun City Paris.'
      })
    });
    const j = await r.json();
