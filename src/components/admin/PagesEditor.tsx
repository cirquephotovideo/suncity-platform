...
function SectionEditor({ section, defaultPageSlug, onClose, onSaved }: {
  ...
  async function aiText() {
    ...
    const r = await fetch('/api/ai/text', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        prompt: `Rédige un paragraphe (80-120 mots) pour une section ${layout} de la page /${pageSlug} du mouvement Sun City Paris. ${title ? `Sujet : ${title}` : ''} Ton inclusif, lumineux, jamais polémique.`
      })
    });
    const j = await r.json();
    setBody(j.text || body);
